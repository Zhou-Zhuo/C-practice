...
int main(int argc, char **argv)
{
	...
	if (argc != 2) {
		fprintf(stderr, "Specific an command.\n");
		return -1;
	}

	strncpy(smd_rw_buff, argv[1], GSERIAL_BUF_LEN - 2);
	strcat(smd_rw_buff, "\r\n");
	...
	printf("Echo %d bytes from android_serial_device:\n%s\n",smd_rw_arg.size, smd_rw_buff);
